...
class DateLapseTest {
    ...
    @Test
    // faltan 3 días, y estamos probando con "últimos 5 días"
    void testDentroDeLosUltimosXDiasAntesDelInicio_retornaTrue() {
        LocalDate inicio = LocalDate.now().plusDays(3);
        DateLapse dl = new DateLapse(inicio, inicio.plusDays(5));
        assertTrue(dl.estaDentroDeLosUltimosXDiasAntesDelInicio(5));
    }
    @Test
    // faltan 10 días, pero solo se cuentan los últimos 5 → debería ser false
    void testFaltanMasDeXDias_retornaFalse() {
        LocalDate inicio = LocalDate.now().plusDays(10);
        DateLapse dl = new DateLapse(inicio, inicio.plusDays(5));
        assertFalse(dl.estaDentroDeLosUltimosXDiasAntesDelInicio(5));
    }
    @Test
    // la reserva empezo ayer
    void testYaComenzo_retornaFalse() {
    ...
    @Test
    // hoy == inicio - 5 días
    void testExactamenteInicioMenosXDias_retornaTrue() {
        LocalDate inicio = LocalDate.now().plusDays(5);
        DateLapse dl = new DateLapse(inicio, inicio.plusDays(3));
        assertTrue(dl.estaDentroDeLosUltimosXDiasAntesDelInicio(5));
    }
    @Test
    // hoy == inicio → la condición "isBefore(getFrom())" es falsa
    void testDiaDeInicio_retornaFalse() {
        LocalDate inicio = LocalDate.now();
        DateLapse dl = new DateLapse(inicio, inicio.plusDays(2));
        assertFalse(dl.estaDentroDeLosUltimosXDiasAntesDelInicio(5));
    }
